...
void CTextureManager::CreateWhiteSquareTexture()
{
	HRESULT hr;
	...
	hr = myDirect3D->GetDevice()->CreateShaderResourceView(tex, NULL, &myWhiteSquareResource);
	if (FAILED(hr))
	{
		return;
	}
...
void DX2D::CTextureManager::Init()
{
	...
	newTexture->myResource = myWhiteSquareResource;
